...
const productsDOM = document.getElementById('products-dom');
...
class Products {
    ...
    async getProducts() {
        try {
            let result = await fetch('https://6628a0ff54afcabd07365b50.mockapi.io/products');
...
class UI {
    ...
    saveCartValues (cart) {
        ...
        //^ Burda butun mehsullarin birge cem qiymetleri ve mehsullarin ne qeder oldugu sayisini gosterdik
        let tempTotal = 0;
        let itemsTotal = 0;
        cart.map((item) => {
        ...
        li.innerHTML = `
            <div class="cart-left">
                ...
                <div class="cart-left-info">
                    <a class="cart-left-info-title"
                        href="#">${item.title}</a>
                    <span class="cart-left-info-price">$
                        ${item.price}</span>
                </div>
            </div>
